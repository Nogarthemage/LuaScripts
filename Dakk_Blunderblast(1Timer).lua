...
function Script.Dakk_Spawn(Unit)
	...
	Script.Path2 = nil
	Script.Phase = nil
	Unit:CreateTimer("Timer", 2500)
end

function Script.Dakk_Update(Unit, mapScript, timeDiff)
	if Script.InactiveTimer == true then
		Unit:UpdateTimers(timeDiff)
		if Unit:IsTimerFinished("Timer") and not Unit:IsInCombat() then
			Script.InactiveTimer = nil
			Script.Path2 = nil
			Script.Phase = nil
			Unit:ResetMovement()
			Unit:ResetTimer("Timer", 2500)
			return
		end
	end
	if Script.InactiveTimer == nil then 
		Unit:UpdateTimers(timeDiff)
		if Script.Phase == nil  and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Unit:PushWaypointMovement(1003)
			Script.Phase = 1
			return
		end
		if Script.Phase == 2 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 3
			Unit:CastSpell(Script.Dummy,7918,false)
			Unit:ResetTimer("Timer", 2500)
			return
		end
		if Script.Phase == 3 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 4
			Unit:SendScriptTextById(11, 330)
			Unit:SendEmote(5)
			Unit:ResetTimer("Timer", 3500)
			return
		end
		if Script.Phase == 4 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 5
			Unit:CastSpell(Script.Dummy,7918,false)
			Unit:ResetTimer("Timer", 2500)
			return
		end
		if Script.Phase == 5 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 6
		...
		if Script.Phase == 6 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 7
			Unit:CastSpell(Script.Dummy,7918,false)
			Unit:ResetTimer("Timer", 2500)
			return
		end
		if Script.Phase == 7 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 8
		...
		if Script.Phase == 8 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 9
			Unit:CastSpell(Script.Dummy,7918,false)
			Unit:ResetTimer("Timer", 2500)
			return
		end
		if Script.Phase == 9 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 10
			Unit:SendScriptTextById(11, 333)
			Unit:SendEmote(22)
			Unit:ResetTimer("Timer", 3000)
			return
		end
		if Script.Phase == 10 and not Unit:IsInCombat() and Unit:IsTimerFinished("Timer") then
			Script.Phase = 11
